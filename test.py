...
def fastDeskew(image, cropped):
    cropped_gray = cv2.cvtColor(cropped, cv2.COLOR_BGR2GRAY)
    skew_h, skew_v = skew_detection(cropped_gray)
    print("校正角度 h ", skew_h, "v", skew_v)
    temporay, M = v_rot(image, int((90 - skew_v) * 1.5), image.shape, 60)
    deskew = h_rot(temporay, skew_h)
    return deskew
...
img = cv2.imread("dataset/0.jpg", cv2.IMREAD_COLOR)
cv2.imshow("img", img)
# gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
# cv2.imshow("gray", gray)
v, M = v_rot(img, int((0)), img.shape, 60)
cv2.imshow("v",v)
# h = h_rot(img, int(180))
# cv2.imshow("h", h)
cv2.waitKey()
